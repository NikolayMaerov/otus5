...
Процедура ЗапуститьВыгрузкуДанныхНаСервере()
	...
	Попытка
		...
		ПараметрыВыгрузки.Вставить("ВыгружатьЗарегистрированныеИзмененияДляУзловПланаОбмена", ВыгружатьЗарегистрированныеИзмененияДляУзловПланаОбмена);
...
Процедура Подключаемый_ПослеПолученияАдресаЗагрузкиФайла(Результат, ДополнительныеПараметры) Экспорт
	...
	ПараметрыПередачи.Вставить("ИмяФайлаВременногоХранилища", ИмяФайлаВременногоХранилища);
	ПараметрыЗапроса = ШаблонПараметровЗапросов();
